...
var _findParentTag = function findParentTag(el, tag) {
  var depth = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
  if (!el) return null;
  if (el.tagName == tag) return el;
  return _findParentTag(el.parentNode, tag, depth + 1);
};
var getCollapsedClientRect = exports.getCollapsedClientRect = function getCollapsedClientRect() {
  ...
  if (!_findParentTag(node, 'P')) return; // only show sidebar at the beginning of an empty <p>
